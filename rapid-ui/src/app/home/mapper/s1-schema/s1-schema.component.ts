...
export class S1SchemaComponent {
  ...
  refreshS1Schemas(): void {
    this.selectedS1SchemaId = "";
    this.filteredS1Schemas = this.s1Schemas
      .filter(s1Schema => {
        const term = this.searchText.toLowerCase();
        return (
          s1Schema.name.toLowerCase().includes(term) ||
          s1Schema.description?.toLowerCase().includes(term)
        );
      })
      .map((s1Schema, i) => ({ ...s1Schema }))
      .slice((this.page - 1) * this.pageSize, (this.page - 1) * this.pageSize + this.pageSize);
  }

  search(): void {
    this.page = 1;
    this.refreshS1Schemas();
  }
  ...
  getAllS1Schemas() {
    ...
    this._s1SchemaService.getAllS1Schemas().subscribe(
      (res: any) => {
        ...
        this.collectionSize = this.s1Schemas.length;
        this.refreshS1Schemas();
      },
      (err) => {
        console.log(err);
