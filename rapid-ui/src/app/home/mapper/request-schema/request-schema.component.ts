...
export class RequestSchemaComponent {
  ...
  refreshRequestSchemas(): void {
    this.selectedRequestSchemaId = "";
    this.filteredRequestSchemas = this.requestSchemas
      .filter(requestSchema => {
        const term = this.searchText.toLowerCase();
        return (
          requestSchema.name.toLowerCase().includes(term) ||
          requestSchema.description?.toLowerCase().includes(term)
        );
      })
      .map((requestSchema, i) => ({ ...requestSchema }))
      .slice((this.page - 1) * this.pageSize, (this.page - 1) * this.pageSize + this.pageSize);
  }

  search(): void {
    this.page = 1;
    this.refreshRequestSchemas();
  }

  getAllRequestSchemas() {
    this.selectedRequestSchemaId = "";
    this._requestSchemaService.getAllRequestSchemas().subscribe(
      (res: any) => {
        this.requestSchemas = res.body;
        this.collectionSize = this.requestSchemas.length;
        this.refreshRequestSchemas();
      },
      (err) => {
        console.log(err);
